...
public:
    RkRunner(std::string_view model_path,
             int npu_id = 1,
             std::size_t model_width = 256,
             std::size_t model_height = 256);
    ~RkRunner();
...
private:
    ...
    cv::Mat makeOutputMat(const std::vector<float> &output, const rknn_tensor_attr &attr) const;
